...
项目使用根目录的 `VERSION` 文件记录当前固件版本号，格式为：
...
固件内置版本由 `main/app/app_version.h` 提供，WebSocket 连接时会通过 `fw_version` 上报给服务器。
...
- `./fw build` 只构建固件。
- `./fw export` 只导出现有 `build/` 目录中的构建产物。
- `./fw release vX.Y.Z` 会要求发布标签和 `VERSION` 文件一致。
- 默认不允许在 Git 工作区有未提交修改时执行正式发布；如需临时允许，可设置 `ALLOW_DIRTY_RELEASE=1`。

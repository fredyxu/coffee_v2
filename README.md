...
| ESP32-S3 开发板 | 1 | 主控 |
| I80 接口 LCD 屏 | 1 | 显示界面 |
| I2C 触摸屏 | 1 | 触摸输入，非必须，但建议选择有触控的显示屏，否则只能使用编码器做交互操作 |
| EC11 旋转编码器 | 1 | 交互操作使用，如果使用触摸屏则编码器非必须使用 |
| 电键接口 | 1 | A/B 两路输入 |
| MAX98357A I2S 音频模块 | 1 | 侧音输出 |
| INMP441 I2S 麦克风模块 | 1 | 麦克风输入 |
| 扬声器 | 1 | 接 MAX98357A 输出，测试使用4欧姆扬声器 |
| 杜邦线 / 排针 / 连接器 | 若干 | 接线使用 |
...
- 操作方式可以选择触控操作，也可以选择编码器操作，两种交互方式均有提供。

## 接线说明

### 接线示意图

#### 注意：下图为项目开发时使用的元件为基础，如果您的元件与作者不同，请勿参考。

![接线示意图](doc/images/pin.png)

[下载接线图PDF文件](doc/images/pin.pdf)
...
说明：
EC11旋转编码器如果没有转接板，通常编码器的四边都有引脚，其中有用的引脚是一边三个，对边是两个。另外两边各有一个比较宽的，可以不用。通常EC11旋转编码器不会标注引脚丝印或名称，区分方法如下：
- 三脚一边，两边的分别是A和B，中间为公共端接 GND。
- 如果旋转方向相反，交换 A/B 即可。
- 连个引脚一边两个引脚功能相同，任取一个接SW，另一个接GND即可。
...
本项目中由作者编写的代码基于 MIT License 开源。你可以自由使用、复制、修改、合并、发布和分发本项目代码，但需要保留原始版权声明和许可证声明。

本项目依赖或引用的第三方开源项目，例如 LVGL、ESP-IDF、FreeRTOS 等，仍遵循其各自的开源许可证。使用、分发或修改本项目时，请同时遵守相关第三方项目的许可证要求。

本项目的 MIT License 正文请查看 [LICENSE](LICENSE) 文件。
